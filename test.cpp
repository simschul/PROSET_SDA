...
// convert indices to c++ format (starting with 0 instead of 1)
// List listOfMatsMinusOne (List x)  {
//   for(int i = 0; i < x.length(); i++) {
//     NumericMatrix mat = as<NumericMatrix>(x[i]);
...
// table0 and table0 needs to be a named list. 1st element: S-matrix, 2nd: A-matrix, all other: egal
List SPD (List table0, List table1, List indices) {
  ...
  // indices = listOfMatsMinusOne(indices); 
  ...
  for(int ilayer = 0; ilayer < n; ilayer++) {
    ...
    if (ilayer > 0) {
      // insert A after S. Change if S should be decomposed as well
      table0.insert(1, a0);
      table1.insert(1, a1);      
    }
